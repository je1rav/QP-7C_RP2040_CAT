/*
 * si5351.c - Si5351 library for RP2040 pico-SDK
 *  
 * Copyright (C) 2023 Hitoshi Kawaji <je1rav@gmail.com> 
 * si5351_init(), si5351_write_bulk(), si5351_write(), si5351_read() 
 * are modified to use with RP2040 pico-SDK.
 * original: https://github.com/NT7S/Si5351
 * 
...
i2c_inst_t * rp2040_i2c_port = RP2040_I2C_PORT;  //i2c port
 ...
 */
void si5351_init(i2c_inst_t * i2c_port)
{
	// Set the i2c port of RP2040 using for Si5351 control 
	rp2040_i2c_port = i2c_port;
 ...
 */
void si5351_set_freq(uint64_t freq, uint32_t pll_freq, enum si5351_clock clk)
{
...
{
	return ref_correction;//return eeprom_read_dword(&ee_ref_correction);
}
...
uint32_t multisynth_calc(uint64_t freq, struct Si5351RegSet *reg)
{
	uint32_t pll_freq;
	uint64_t lltmp;
...
uint32_t multisynth_recalc(uint64_t freq, uint32_t pll_freq, struct Si5351RegSet *reg)
{
	uint64_t lltmp;
	uint32_t rfrac, denom, a, b, c, p1, p2, p3;
	uint8_t divby4;
...
uint8_t si5351_write_bulk(uint8_t addr, uint8_t bytes, uint8_t *data)
{
    ...
    i2c_write_blocking(rp2040_i2c_port, SI5351_BUS_ADDR, buf, bytes, false);
	return 0;
}
...
uint8_t si5351_write(uint8_t addr, uint8_t data)
{
	...
	buf[0] = addr;
	buf[1] = data;
    i2c_write_blocking(rp2040_i2c_port, SI5351_BUS_ADDR, buf, 2, false);
	return 0;
}

uint8_t si5351_read(uint8_t addr, uint8_t *data)
{
	i2c_write_blocking(rp2040_i2c_port, SI5351_BUS_ADDR, &addr, 1, true);  // true to keep master control of bus
    i2c_read_blocking(rp2040_i2c_port, SI5351_BUS_ADDR, data, 1, false);  // false - finished with bus 
	return 0;	
}
