/*
 * si5351.h - Si5351 library for RP2040 pico-SDK
 *  
 * Copyright (C) 2023 Hitoshi Kawaji <je1rav@gmail.com> 
 *  A few Modifieations were done to use with RP2040 pico-SDK.
...
/* Define definitions */
#define RP2040_I2C_PORT                     i2c0  //default RP2040 i2c port (i2c0 or i2c1)

#define SI5351_BUS_ADDR	                    0x60  //Si5351 i2c address
...
#define SI5351_MULTISYNTH_MIN_FREQ			440000
...
# define do_div(n,base) ({                                      \
        ...
        uint32_t __rem;                                         \
        __rem = (uint32_t)(((uint64_t)(n)) % __base);                       \
        (n) = ((uint64_t)(n)) / __base;                         \
        __rem;                                                  \
 })
...
void si5351_init(i2c_inst_t *);
void si5351_set_freq(uint64_t, uint32_t, enum si5351_clock);
...
uint32_t multisynth_calc(uint64_t, struct Si5351RegSet *);
uint32_t multisynth_recalc(uint64_t, uint32_t,struct Si5351RegSet *);
